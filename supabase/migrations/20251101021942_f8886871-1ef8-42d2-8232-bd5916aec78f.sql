-- [Phase 77-UF-FIX.3] Replace Safe Transaction Rebuild
-- Purpose: Prevent FK violations and Realtime conflicts during Replace mode

-- 1. Create temporary log table without FK constraints
CREATE TABLE IF NOT EXISTS public.participants_log_temp (
...
CREATE INDEX IF NOT EXISTS idx_participants_log_temp_event ON public.participants_log_temp(event_id);
CREATE INDEX IF NOT EXISTS idx_participants_log_temp_participant ON public.participants_log_temp(participant_id);

-- 2. Rebuild ai_participant_import_from_excel with safe transaction handling
...
BEGIN
  ...
  -- Replace mode: Safe transaction handling
  IF p_replace THEN
    ...
    -- Temporarily disable triggers to prevent FK violations
    EXECUTE 'ALTER TABLE participants_log DISABLE TRIGGER ALL';
    ...
    -- Re-enable triggers
    EXECUTE 'ALTER TABLE participants_log ENABLE TRIGGER ALL';
  END IF;
...
-- 3. Strengthen trigger protection
...
DECLARE
  ...
  v_agency_id UUID;
BEGIN
  -- Protection: Skip if participant_id is NULL or participant doesn't exist
