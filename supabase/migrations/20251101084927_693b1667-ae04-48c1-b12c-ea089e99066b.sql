-- [Phase 78-B.10] Update phone validation error message to match QA spec
...
BEGIN
  ...
  LOOP
    ...
    -- Phone normalization and validation (QA spec: simplified message)
    IF v_row.phone IS NOT NULL AND trim(v_row.phone) != '' THEN
      ...
      IF v_normalized_phone != trim(v_row.phone) THEN
        UPDATE participants_staging 
        SET validation_status = 'error', 
            validation_message = '형식이 올바르지 않습니다.'
        WHERE id = v_row.id;
        v_error_count := v_error_count + 1;
        CONTINUE;
      END IF;
