-- Phase 77-UF-FIX.1: participants_log FK protection + Replace mode stabilization
-- Purpose: Prevent FK violations during Replace mode by enhancing trigger logic

-- 1. Enhanced trigger: Skip when participant_id is NULL or participant doesn't exist
CREATE OR REPLACE FUNCTION public.set_participants_log_context()
RETURNS TRIGGER AS $$
BEGIN
  -- Skip trigger if participant_id is NULL (bulk operations)
  IF NEW.participant_id IS NULL THEN
    RETURN NEW;
  END IF;
  
  -- Skip trigger if participant doesn't exist (deleted participant case)
  IF NOT EXISTS (SELECT 1 FROM public.participants WHERE id = NEW.participant_id) THEN
    RETURN NEW;
  END IF;
  
  -- Lookup event_id and agency_id only for existing participants
...
-- 2. Add pg_notify to RPC for frontend notification
...
BEGIN
  ...
  IF p_replace = true THEN
    ...
    -- Log deletion with explicit fields
  ...
  -- Real-time notification for upload completion
  PERFORM pg_notify(
    'participants_upload',
    json_build_object(
      'event_id', p_event_id,
  ...
  -- Notify frontend that replace mode is done
  IF p_replace = true THEN
    PERFORM pg_notify('participants_replace_done', p_event_id::text);
  END IF;
