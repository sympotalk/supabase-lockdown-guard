-- Phase 77-RPC-REPLACE-FK-FINAL.v5
-- Purpose: Add ON DELETE CASCADE to FK and simplify replace logic
...
-- 2. Recreate FK with CASCADE deletion
ALTER TABLE public.participants_log
...
COMMENT ON CONSTRAINT participants_log_participant_id_fkey ON public.participants_log 
IS 'Automatically deletes related log entries when a participant is deleted';

-- 3. Update RPC to rely on CASCADE (no manual log deletion needed)
