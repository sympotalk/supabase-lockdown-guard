-- [71-I.QA3-FIX.R9] RPC fully synced with rebuilt participants table

-- Drop existing function
DROP FUNCTION IF EXISTS fn_bulk_upload_participants CASCADE;

-- Recreate function aligned with new participants structure
...
BEGIN
  ...
  SELECT agency_id INTO v_user_agency FROM user_roles WHERE user_id = auth.uid() LIMIT 1;
  ...
  LOOP
    ...
    ELSE
      INSERT INTO participants (
        event_id, agency_id, name, organization, phone, email, memo, created_by
      ) VALUES (
        p_event_id, v_event_agency, v_name, v_org, v_phone, v_email, v_memo, auth.uid()
      );
      v_new := v_new + 1;
    END IF;
...
COMMENT ON FUNCTION fn_bulk_upload_participants IS '[71-I.QA3-FIX.R9] RPC fully synced with rebuilt participants table.';
