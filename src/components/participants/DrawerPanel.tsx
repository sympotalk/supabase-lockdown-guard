...
import { RoleBadgeSelector } from "./RoleBadgeSelector";
        ...
        <div className="px-4 py-3 space-y-3">
          ...
          <div id="badge-section">
            <DrawerSection title="구분 (Badge)" icon={<Award className="h-4 w-4" />} defaultOpen>
              <RoleBadgeSelector
                fixedRole={localData.fixed_role}
                customRole={localData.custom_role}
                onFixedRoleChange={async (newRole) => {
                  setLocalData({ ...localData, fixed_role: newRole || undefined });
                  
                  // [Phase 73-L.7.25] Update both fixed_role and role_badge
                  const { error } = await supabase
                    .from("participants")
                    .update({ 
                      fixed_role: newRole,
                      role_badge: newRole || "참석자", // ✅ role_badge 필드에도 반영
                      last_edited_by: user?.id,
                      last_edited_at: new Date().toISOString()
                    })
                    .eq("id", localData.id);
                  ...
                  if (error) {
                    console.error("[Phase 73-L.7.25] Error updating fixed_role:", error);
                    toast.error("저장에 실패했습니다. 다시 시도해주세요.");
                  } else {
                    // Log activity with valid type
                    await supabase.from("activity_logs").insert({
                      title: "구분 변경",
                ...
                onCustomRoleChange={async (trimmed) => {
                  ...
                  // [Phase 73-L.7.25] Update both custom_role and role_badge
                  const { error } = await supabase
                    .from("participants")
                    .update({ 
                      custom_role: trimmed || null,
                      role_badge: trimmed || localData.fixed_role || "참석자", // ✅ role_badge 필드에도 반영
                      last_edited_by: user?.id,
                      last_edited_at: new Date().toISOString()
                    })
                    .eq("id", localData.id);
                  ...
                  if (error) {
                    console.error("[Phase 73-L.7.25] Error updating custom_role:", error);
                    toast.error("저장에 실패했습니다. 다시 시도해주세요.");
                  } else {
                    // Log activity with valid type
                    await supabase.from("activity_logs").insert({
